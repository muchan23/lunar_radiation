...
void MyDetectorConstruction::DefineMaterials()
{
    ...
    //月のレゴリスの構成を定義
    lunarRegolith = new G4Material("LunarRegolith", 1.5*g/cm3, 3);
    lunarRegolith->AddElement(nist->FindOrBuildElement("Si"), 50*perCent);
    lunarRegolith->AddElement(nist->FindOrBuildElement("O"), 40*perCent);
    lunarRegolith->AddElement(nist->FindOrBuildElement("Fe"), 10*perCent);

    G4MaterialPropertiesTable *mptLunarRegolith = new G4MaterialPropertiesTable();
    mptLunarRegolith->AddProperty("REFLECTIVITY", energy, reflectivity, 2);

    // 透過率の設定（例として0.1とする）
    G4double transmission[2] = {0.2, 0.2}; // 透過率（20%）
    mptLunarRegolith->AddProperty("TRANSMITTANCE", energy, transmission, 2);

    lunarRegolith->SetMaterialPropertiesTable(mptLunarRegolith);
}

//月の地面
void MyDetectorConstruction::ConstructGround()
{
    G4double groundThickness = 450.0 * km; // 地面の厚さ
    solidGround = new G4Box("solidGround", 600.0 * km, 600.0 * km, groundThickness / 2.0);
    logicGround = new G4LogicalVolume(solidGround, lunarRegolith, "logicGround");

    // 月面の層に色を設定
    G4VisAttributes *visAttributes = new G4VisAttributes(G4Colour(0.9, 0.9, 0.9)); // グレー色
    logicGround->SetVisAttributes(visAttributes);
    ...
    // エネルギー範囲と反射率を宣言
    G4double energy[2] = {0.1*eV, 10.0*eV}; // エネルギー範囲
    G4double reflectivity[2] = {0.9, 0.9}; // 反射率（90%）

    // 光学表面に反射率を設定
    G4MaterialPropertiesTable *mptMirrorSurface = new G4MaterialPropertiesTable();
    mptMirrorSurface->AddProperty("REFLECTIVITY", energy, reflectivity, 2);
    mirrorSurface->SetMaterialPropertiesTable(mptMirrorSurface);

    G4LogicalSkinSurface *skin = new G4LogicalSkinSurface("skin", logicGround, mirrorSurface);

    // 地面をワールド座標系に配置
    physGround = new G4PVPlacement(0, G4ThreeVector(0, 0, -groundThickness / 2.0), logicGround, "physGround", logicWorld, false, 0, true);
}
